...
export default class RagnoMemoryStore extends MemoryStore {
        ...
        const triples = Array.from(concepts).map(label => {
            ...
            return `
                <${uri}> a ragno:Entity ;
                    ...
                    mcp:frequency 1 ;
                    mcp:firstSeen "${now}"^^xsd:dateTime ;
                    mcp:lastAccessed "${now}"^^xsd:dateTime .
            `;
        }).join('\n');
    ...
    buildBatchFrequencyUpdate(conceptMap) {
        const updates = Array.from(conceptMap.entries()).map(([label, increment]) => {
            ...
            return `
                DELETE {
                    <${uri}> mcp:frequency ?oldFreq ;
                             mcp:lastAccessed ?oldAccess .
                }
                INSERT {
                    <${uri}> mcp:frequency ?newFreq ;
                             mcp:lastAccessed "${new Date().toISOString()}"^^xsd:dateTime .
                }
                WHERE {
                    <${uri}> mcp:frequency ?oldFreq .
                    OPTIONAL { <${uri}> mcp:lastAccessed ?oldAccess }
            ...
            updates.push(`
                ...
                INSERT {
                    <${relUri}> a ragno:Relationship ;
                        ...
                        mcp:lastUpdated "${now}"^^xsd:dateTime ;
                ...
                DELETE {
                    <${relUri}> ragno:hasWeight ?oldWeight ;
                               mcp:cooccurrenceCount ?oldCount ;
                               mcp:lastUpdated ?oldUpdate .
                }
                INSERT {
                    <${relUri}> ragno:hasWeight ?newWeight2 ;
                               mcp:cooccurrenceCount ?newCount2 ;
                               mcp:lastUpdated "${now}"^^xsd:dateTime .
                }
                WHERE {
                    <${relUri}> ragno:hasWeight ?oldWeight ;
                               mcp:cooccurrenceCount ?oldCount .
                    OPTIONAL { <${relUri}> mcp:lastUpdated ?oldUpdate }
